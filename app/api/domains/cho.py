...
async def bancho_http_handler() -> Response:
    ...
    return HTMLResponse(
        f"""
<!DOCTYPE html>
<body style="font-family: monospace; white-space: pre-wrap;">HAPPY CHEATING!

<a href="online">{len(players)} ONLINE:</a>
...
async def bancho_handler(
    ...
    if not player:
        ...
        return Response(
            content=(
                app.packets.notification("SERVER REBOOTED, SORRY!!!")
...
WELCOME_MSG = "\n".join(
    (
        ...
        "WE RUNNING ON BANCHO.PY AND GUWEB [for now]",
        "JOIN OUR (DISCORD SERVER)[https://dsc.gg/4ayo]!",
        "HAPPY CHEATING!",
    ),
)

RESTRICTED_MSG = (
    "YOU HAVE BEN BANNED FROM 4ayo OSU SERVER"
    "MISTAKE? CONTACT US ON DISCORD #tickets!"
)

WELCOME_NOTIFICATION = app.packets.notification(
    f"YOU ARE PLAYING ON {BASE_DOMAIN}! :3",
)

OFFLINE_NOTIFICATION = app.packets.notification(
    "SERVER RUNNING IN MODE: OFFLINE; "
    "SOME THINGS CAN BE UNAVALIBE FN :( )",
)
